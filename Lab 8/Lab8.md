...
> This update converts the HR Agent into a persistent, cloud-hosted
> agent within Microsoft Foundry. It connects to the Foundry project
> using AIProjectClient, reuses the existing "Enterprise-HRAgent" if
> deployed, or creates a new one with specialized HR domain
> instructions. Once deployed, it’s wrapped in a ChatAgent linked via
> AzureAIAgentClient, enabling stateful, reusable, and centrally managed
> HR automation within the Foundry environment.

10. Once done, select **File** **(1)** and then
    click **Save** **(2)** to save the file.
...
14. Once done, select **File** **(1)** and then
    click **Save** **(2)** to save the file.
...
25. Once navigated to the Microsoft Foundry Portal, select **Agents
    (1)** from the left menu. You will see that all the agents got
